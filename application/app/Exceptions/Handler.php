...
namespace TheFarrelHotel\Exceptions;
...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Exception $exception)
    {
        if(!config('app.debug') && auth()->check() && !$exception instanceof \Illuminate\Validation\ValidationException) {
            view()->share([
                'exception' => $exception,
                'menus' => \TheFarrelHotel\Http\Models\Pengaturan\Menu::getMenus(auth()->user()->level_id),
                'user' => auth()->user(),
                'title' => "Error"
            ]);
            ...
            if($statusCode >= 500)
                return response()->view('errors-app.5xx', [], $statusCode);

            if($statusCode >= 400)
                return response()->view('errors-app.4xx', [], $statusCode);
        }
